...
def render_welcome_panel():
    ...
    st.write(
        """
        EduVisLab анализирует образовательное видео и формирует отчёт
        о содержании, техническом качестве, стиле подачи и возможных барьерах восприятия.
        """
    )

    st.info(
        "🚀 Быстрый вариант: нажмите **«Демо-режим: готовые результаты»** слева. "
        "В демо уже подготовлены отчёты по 16 видео из валидационного набора, "
        "поэтому они открываются сразу — без запуска полного пайплайна и ожидания обработки.\n\n"
        "Все демо-видео заранее обработаны с одинаковыми параметрами: "
    ...
    st.write(
        """
        1. Вставьте ссылку на YouTube-видео в левом блоке.  
        2. Выберите роль: обучающийся, спикер, продакшн или заказчик.  
        3. Укажите уровень образования.  
        4. Отметьте уровень погружённости и цель просмотра.  
        5. Нажмите **«Продолжить»**.
        """
    )

    st.markdown("### Что появится в отчёте")
    st.write(
        """
        - краткое описание ценности видео;
        - темы и таймкоды;
        - пререквизиты и пробелы покрытия;
        - профиль подачи: академичность и инструментальность;
        - техническое качество видео;
        - флаги восприятия по логике, сенсорике, интуиции и эмоции.
        """
    )

    st.info(
        "Пока анализ не запущен, здесь отображается краткая инструкция. "
        "После запуска справа появится отчёт по выбранному видео."
    )

    st.info(
        """
        Если вы запускаете анализ не через демо-режим, система покажет экран ожидания с текущим этапом обработки. 
        Для ориентира: видео длиной около 20 минут на Colab L4 обычно обрабатывается примерно за 5–7 минут.
        """
    )
